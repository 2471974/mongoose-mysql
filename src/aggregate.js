...
class Aggregate {
  ...
  buildProject (project, prefix) {
    ...
    if (typeof project === 'string') {
      let index = prefix, value = project
      if (value.indexOf('$') === 0) {
    ...
    for (let index in project) {
      ...
      if (Object.prototype.toString.call(value) === '[object Object]') {
        ...
        value = Object.keys(value).map(item => {return value[item]})[0].substring(1)
        switch (fn) {
  ...
  buildQuery (options, subquery, data) {
    ...
    let sql = ["select ", Object.keys(project).length > 0 ? Object.keys(project).map(item => {return project[item]}).join(', ') : "*"]
  ...
  buildGroup (group, subquery, data) {
    ...
    for (let key in group) {
      ...
      value = Object.keys(value).map(item => {return value[item]})[0]
      switch (fn) {
        case '$sum':
        case '$max':
        case '$min':
    ...
    if (Object.keys(project).length > 0) {
      sql.push('select ', Object.keys(project).map(key => {return project[key]}).join(', '))
    } else {
      sql.push('select *')
    }
    sql.push(' from (', subquery, ') as `', this.tableName(), '`')
