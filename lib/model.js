...
var Model = function (_Document) {
  ...
  _createClass(Model, null, [{
    ...
    key: 'aggregate',
    value: function aggregate(options, callback) {
    ...
    key: 'update',
    value: function update(condition, doc, options, callback) {
      var _this2 = this;
      ...
      return this.query().where(condition).distinct('_id').exec().then(function (ids) {
        if (ids.length === 0) {
          // 没有匹配记录
          if (options.upsert) {
            // 执行插入
            return _this2.save(doc, callback);
          } else {
            // 返回受影响条数
            callback && callback(null, ids.length);
            return _index3.default.Promise.resolve(ids.length);
          }
        }
        if (!options.multi) ids = [ids[0]]; // 单记录更新
        if (options.overwrite) {
          // 覆盖模式
          var promise = _this2.removeById(ids); // 删除原记录
          ids.forEach(function (id) {
            // 插入新记录
            promise = promise.then(function () {
              _this2.save(Object.assign({ _id: id }, doc));
            });
          });
          ...
          // 局部更新模式
          if (Object.keys(doc).filter(function (key) {
        ...
        var mapping = _this2.mapping();
        var queries = [],
            tables = _schema2.default.update(doc, mapping.mappings);
        var tableName = Object.keys(mapping.tables).shift();
        for (var index in mapping.tables) {
          ...
          queries.push(_index3.default.connection.query(sql.join(''), table.data.concat(ids)));
        }
        return _index3.default.connection.beginTransaction().then(function () {
          // 启用事务
          return _index3.default.Promise.all(queries);
        }).then(function (result) {
          // 提交事务
          return _index3.default.connection.commit();
        }).catch(function (error) {
          callback && callback(error);
          _index3.default.connection.rollback(); // 回滚事务
          return _index3.default.Promise.reject(error);
        }).then(function (result) {
          if (options.new) {
            return _this2.findById(options.multi ? ids : ids.shift(), options.select, callback);
          } else {
            callback && callback(null, ids.length);
            return _index3.default.Promise.resolve(ids.length);
          }
    ...
    value: function remove(condition, callback) {
      ...
      return this.query().where(condition).distinct('_id').exec().then(function (result) {
        if (result.length === 0) {
          ...
          return _index3.default.Promise.resolve(result.length);
        }
        return _this3.removeById(result, callback);
      }).catch(function (error) {
        callback && callback(error);
        return _index3.default.Promise.reject(error);
      });
    ...
    value: function removeById(id, callback) {
      ...
      var promise = _index3.default.connection.beginTransaction().then(function () {
        // 启用事务
        return _index3.default.connection.query(query.sql, query.data); // 删除主文档
      });
      queries.forEach(function (query) {
        // 删除子文档
        promise = promise.then(function (result) {
          return new _index3.default.Promise(function (resolve, reject) {
            _index3.default.connection.query(query.sql, query.data).then(function () {
      ...
      return promise.then(function (result) {
        // 提交事务
        return new _index3.default.Promise(function (resolve, reject) {
          _index3.default.connection.commit().then(function () {
            resolve(result); // 保留主文档执行结果
          }).catch(function (error) {
            return reject(error);
          });
        ...
        return _index3.default.Promise.resolve(result.affectedRows);
      });
    }
  }, {
    key: 'tables',
    value: function tables(fields, mapping) {
      ...
      if (Object.prototype.toString.call(fields) !== '[object Object]') {
        ...
        fields.split(' ').forEach(function (item) {
          ...
          if (item.indexOf('-') === 0) {
            isExclude = true;
            Object.assign(doc, _defineProperty({}, item.substring(1), -1));
          } else if (item.indexOf('+') === 0) {
            Object.assign(doc, _defineProperty({}, item.substring(1), 1));
          } else if (item !== '') {
            Object.assign(doc, _defineProperty({}, item, 1));
          }
        });
        fields = doc;
      } else {
      ...
      for (var field in fields) {
        var item = mapping.mappings[field];
      ...
      var _loop = function _loop(_index) {
        if (typeof tables[_index] === 'undefined') {
          if (isExclude) Object.assign(result, _defineProperty({}, _index, mapping.tables[_index]));
          return 'continue';
        }
        var table = mapping.tables[_index];
        Object.assign(result, _defineProperty({}, _index, {
          columns: table.columns.filter(function (item) {
            if (isExclude) {
              return tables[_index].indexOf(item) === -1;
            } else {
              return tables[_index].indexOf(item) !== -1;
            }
          }),
        ...
        if (_index === tableName) {
          result[_index].columns.indexOf('_id') === -1 && result[_index].columns.unshift('_id');
        } else {
          result[_index].columns.indexOf('autoIndex') === -1 && result[_index].columns.unshift('autoIndex');
          result[_index].columns.indexOf('autoId') === -1 && result[_index].columns.unshift('autoId');
        }
      };

      for (var _index in mapping.tables) {
    ...
    value: function findById(id, fields, callback) {
      ...
      var tableName = Object.keys(mapping.tables)[0]; // 主表
      var tables = this.tables(fields, mapping),
          queries = [],
          ids = id instanceof Array ? id : [id];
      for (var table in tables) {
        var sql = [];
        ...
        queries.push(_index3.default.connection.query(sql.join(''), ids));
      }
      return _index3.default.Promise.all(queries).then(function (results) {
        var data = [];
        ids.forEach(function (id) {
          var rindex = 0,
              doc = null;
          ...
          if (doc !== null) data.push(_this4.new(doc));
        });
        if (!(id instanceof Array)) data = data.length > 0 ? data[0] : data;
        callback && callback(null, data);
        return _index3.default.Promise.resolve(data);
      }).catch(function (error) {
        callback && callback(error);
        return _index3.default.Promise.reject(error);
      });
    ...
    value: function save(doc, callback) {
      ...
      var promise = _index3.default.connection.beginTransaction().then(function () {
        // 启用事务
        return _index3.default.connection.query(query.sql, query.data); // 插入主文档
      });
      queries.forEach(function (query) {
        // 插入子文档
        promise = promise.then(function (result) {
      ...
      return promise.then(function (result) {
        return _this5.findById(result.insertId, callback);
      });
    }
  }, {
    key: 'ddl',
