...
class Model extends _document2.default {
  ...
  validate(options, callback) {
    ...
    return _asyncToGenerator(function* () {
      ...
      _this2.doPre('validate');
      let fields = _this2.$schema().fields;
      try {
        for (let field in fields) {
          let item = fields[field];
          if (options.default && typeof item.default != 'undefined' && typeof _this2[field] === 'undefined') {
            _this2[field] = item.default;
          }
          if (typeof item.required != 'undefined') {
            if (!_this2[field]) _this2.deny(_this2.$name(), field, _index2.default.Error.messages.general.required, [{ regexp: '{PATH}', replacement: field }, { regexp: '{VALUE}', replacement: _this2[field] }]);
          }
          if (typeof item.min != 'undefined') {
            if (_this2[field] < item.min) _this2.deny(_this2.$name(), field, _index2.default.Error.messages.Number.min, [{ regexp: '{PATH}', replacement: field }, { regexp: '{VALUE}', replacement: _this2[field] }, { regexp: '{MIN}', replacement: item.min }]);
          }
          if (typeof item.max != 'undefined') {
            if (_this2[field] > item.max) _this2.deny(_this2.$name(), field, _index2.default.Error.messages.Number.max, [{ regexp: '{PATH}', replacement: field }, { regexp: '{VALUE}', replacement: _this2[field] }, { regexp: '{MAX}', replacement: item.max }]);
          }
          if (typeof item.enum != 'undefined') {
            if (item.enum.indexOf(_this2[field]) === -1) _this2.deny(_this2.$name(), field, _index2.default.Error.messages.String.enum, [{ regexp: '{PATH}', replacement: field }, { regexp: '{VALUE}', replacement: _this2[field] }]);
          }
        }
        return _this2;
      } catch (e) {
