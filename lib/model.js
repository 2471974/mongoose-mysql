      ...
      try {
        for (let field in fields) {
          ...
          if (typeof item.required != 'undefined') {
            if (!_this[field]) _this.deny(_this.name(), field, _index2.default.Error.messages.general.required, [{ regexp: '{PATH}', replacement: field }, { regexp: '{VALUE}', replacement: _this[field] }]);
          }
          if (typeof item.min != 'undefined') {
            if (_this[field] < item.min) _this.deny(_this.name(), field, _index2.default.Error.messages.Number.min, [{ regexp: '{PATH}', replacement: field }, { regexp: '{VALUE}', replacement: _this[field] }, { regexp: '{MIN}', replacement: item.min }]);
          }
          if (typeof item.max != 'undefined') {
            if (_this[field] > item.max) _this.deny(_this.name(), field, _index2.default.Error.messages.Number.max, [{ regexp: '{PATH}', replacement: field }, { regexp: '{VALUE}', replacement: _this[field] }, { regexp: '{MAX}', replacement: item.max }]);
          }
          if (typeof item.enum != 'undefined') {
            if (item.enum.indexOf(_this[field]) === -1) _this.deny(_this.name(), field, _index2.default.Error.messages.String.enum, [{ regexp: '{PATH}', replacement: field }, { regexp: '{VALUE}', replacement: _this[field] }]);
          }
        }
      } catch (e) {
