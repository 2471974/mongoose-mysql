...
class Model extends _document2.default {
  ...
  static new(doc) {
    ...
    for (let key in schema.fields) {
      ...
      Object.assign(virtuals, { [key]: new _schema4.default.VirtualType(key).set(schema.fields[key].set) });
    }
        ...
        switch (action) {
          ...
          case '$pull':
            ...
            for (let field in obj) {
              ...
              yield _index2.default.connection.query(query.sql, query.data);
            }
            break;
        }
      }
      ...
      return this.rebuildArray(ids, doc).then(doc => {
        ...
        for (let index in mapping.tables) {
          ...
          ids.forEach(id => {
            ...
            queries.push(_index2.default.connection.query(sql.join(''), [id].concat(table.idata, table.data)));
          });
        }
