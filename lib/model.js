...
class Model extends _document2.default {
    ...
    return _asyncToGenerator(function* () {
      ...
      for (let key in doc) {
        ...
        switch (action) {
          ...
          case '$pull':
            // 单值和条件
            let where = _this2.query().buildWhere(obj[field], field);
            console.log(where);
            break;
        }
  ...
  validate(options, callback) {
    ...
    return _asyncToGenerator(function* () {
      ...
      _this3.doPre('validate');
      let fields = _this3.$schema().fields;
      try {
        for (let field in fields) {
          let item = fields[field];
          if (options.default && typeof item.default != 'undefined' && typeof _this3[field] === 'undefined') {
            _this3[field] = item.default;
          }
          if (typeof item.required != 'undefined') {
            if (!_this3[field]) _this3.deny(_this3.$name(), field, _index2.default.Error.messages.general.required, [{ regexp: '{PATH}', replacement: field }, { regexp: '{VALUE}', replacement: _this3[field] }]);
          }
          if (typeof item.min != 'undefined') {
            if (_this3[field] < item.min) _this3.deny(_this3.$name(), field, _index2.default.Error.messages.Number.min, [{ regexp: '{PATH}', replacement: field }, { regexp: '{VALUE}', replacement: _this3[field] }, { regexp: '{MIN}', replacement: item.min }]);
          }
          if (typeof item.max != 'undefined') {
            if (_this3[field] > item.max) _this3.deny(_this3.$name(), field, _index2.default.Error.messages.Number.max, [{ regexp: '{PATH}', replacement: field }, { regexp: '{VALUE}', replacement: _this3[field] }, { regexp: '{MAX}', replacement: item.max }]);
          }
          if (typeof item.enum != 'undefined') {
            if (item.enum.indexOf(_this3[field]) === -1) _this3.deny(_this3.$name(), field, _index2.default.Error.messages.String.enum, [{ regexp: '{PATH}', replacement: field }, { regexp: '{VALUE}', replacement: _this3[field] }]);
          }
        }
        return _this3;
      } catch (e) {
