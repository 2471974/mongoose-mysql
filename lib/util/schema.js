...
exports.default = {
  ...
  update: function update(data, mappings, parent) {
    var result = {};
    for (var key in data) {
      var value = data[key];
      if (key.indexOf('$') === 0) {
        var r = this.update(value, mappings, key);
        for (var index in r) {
          if (result[index]) {
            var _result$index$fields, _result$index$data;

            (_result$index$fields = result[index].fields).push.apply(_result$index$fields, _toConsumableArray(r[index].fields));
            (_result$index$data = result[index].data).push.apply(_result$index$data, _toConsumableArray(r[index].data));
          } else {
      ...
      var lparent = parent.toLowerCase(),
          lkey = mappings[key];
      if (!lkey) throw 'can not mapping ' + key;
      var table = result[lkey.table] || (result[lkey.table] = { fields: [], data: [] });
      switch (lparent) {
        case '$inc':
  ...
  mapping: function mapping(fields, tableName, autoIndex) {
    ...
    result.tables = Object.assign(_defineProperty({}, tableName, { columns: columns, maps: maps, isArray: isArray }), result.tables);
